"""Unit tests for `xauusd_snd_qm_structure_d1_v2.py` (patched, VALIDATED —
not yet active) — XAUUSD S&D V1/V2 + Quasimodo + market-structure strategy,
D1 Position preset. Zone timeframe is W1, resampled in-strategy from D1
(7 D1 bars per bucket).
...
from src.strategies.generated.xauusd_snd_qm_structure_d1_v2 import (
...
STRATEGY_PATH = Path("src/strategies/generated/xauusd_snd_qm_structure_d1_v2.py")

START = datetime(2026, 1, 1, tzinfo=UTC)
STEP = timedelta(days=1)
BUCKET_SIZE = 7  # zone_tf_minutes(10080) / entry_tf_minutes(1440)
SPREAD_POINTS = 20.0
...
def _rbr_fixture() -> list[dict]:
    """~20 flat W1 buckets for ATR warmup, then a two-candle rally
    (leg-in) - base - two-candle rally (leg-out): an RBR demand zone
    at [101.8, 102.1]. Mirrors the H1 fixture's shape at daily/weekly
    scale."""
    bars: list[dict] = []
    for _ in range(20):
...
def _strategy() -> XauusdSndQmStructureD1:
    return XauusdSndQmStructureD1()
...
def test_spec_shape() -> None:
    strategy = _strategy()
    assert strategy.spec.name == "xauusd_snd_qm_structure_d1"
    assert strategy.spec.symbols == ("XAUUSD",)
    assert strategy.spec.entry_timeframe == "D1"
    assert strategy.spec.htf_veto is False
    assert strategy.spec.close_on_opposite_signal is True
    params = strategy.spec.params
    assert params["zone_tf_minutes"] == 10080
    assert params["entry_tf_minutes"] == 1440
...
def strategy_params() -> dict:
    return XauusdSndQmStructureD1().spec.params
...
def test_resample_aggregates_and_drops_partial_bucket() -> None:
    ...
    result = _resample(pd.DataFrame(bars), 10080, 1440)
    assert result is not None
...
def test_detect_zones_v1_finds_rbr_demand_zone() -> None:
    ...
    resampled = _resample(frame, 10080, 1440)
