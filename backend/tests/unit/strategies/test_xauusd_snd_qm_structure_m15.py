"""Unit tests for `xauusd_snd_qm_structure_m15_v5.py` (patched, VALIDATED —
not yet active) — XAUUSD S&D V1/V2 + Quasimodo + market-structure strategy,
M15 Intraday preset. Zone timeframe is H1, resampled in-strategy from M15
(4 M15 bars per bucket).
...
from src.strategies.generated.xauusd_snd_qm_structure_m15_v5 import (
    XauusdSndQmStructureM15,
    _atr,
...
STRATEGY_PATH = Path("src/strategies/generated/xauusd_snd_qm_structure_m15_v5.py")

START = datetime(2026, 1, 1, tzinfo=UTC)
STEP = timedelta(minutes=15)
BUCKET_SIZE = 4  # zone_tf_minutes(60) / entry_tf_minutes(15)
SPREAD_POINTS = 20.0
...
def _rbr_fixture() -> list[dict]:
    """~20 flat H1 buckets for ATR warmup (range 1.2, body 0.4 -> ATR~1.2),
    ...
    clear of S&D V1's leg threshold (0.9 * ATR) via the cumulative 1.6
    run move, but individually below S&D V2's engulfing threshold
    (1.3 * ATR) so no duplicate V2 zone forms over the same band."""
    bars: list[dict] = []
    for _ in range(20):
...
def _strategy() -> XauusdSndQmStructureM15:
    return XauusdSndQmStructureM15()


def _ctx(bars: list[dict]) -> MarketContext:
    return MarketContext(
        symbol="XAUUSD", candles={"M15": pd.DataFrame(bars)}, spread_points=SPREAD_POINTS
    )


def test_spec_shape() -> None:
    strategy = _strategy()
    assert strategy.spec.name == "xauusd_snd_qm_structure_m15"
    assert strategy.spec.symbols == ("XAUUSD",)
    assert strategy.spec.entry_timeframe == "M15"
    assert strategy.spec.htf_veto is False
    assert strategy.spec.close_on_opposite_signal is True
    params = strategy.spec.params
    assert params["zone_tf_minutes"] == 60
    assert params["entry_tf_minutes"] == 15
...
def strategy_params() -> dict:
    return XauusdSndQmStructureM15().spec.params
...
def test_resample_aggregates_and_drops_partial_bucket() -> None:
    ...
    result = _resample(pd.DataFrame(bars), 60, 15)
    assert result is not None
...
def test_detect_zones_v1_finds_rbr_demand_zone() -> None:
    ...
    resampled = _resample(frame, 60, 15)
