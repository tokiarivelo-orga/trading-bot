"""Train the SMC exit-aware model with walk-forward chronological validation.

Run from ``backend/``::

    uv run python scripts/train_smc_dl_v2.py --symbol XAUUSD
    uv run python scripts/train_smc_dl_v2.py --symbol XAUUSD --folds 5 --epochs 12

What this does that ``train_smc_dl.py`` does not
────────────────────────────────────────────────────────────────────────
* **Walk-forward, never a single split.** ``train_smc_dl`` has one temporal
  cut at ``START_DATE`` and reports the val loss at that one cut. One cut on
  autocorrelated financial data is one draw from a very wide distribution.
  Here the series is cut into ``--folds`` expanding windows, each trained on
  everything before its OOS block and scored only on that block, and the
  spread across folds is reported. A per-hour entry filter on this exact data
  measured PF 2.70 in-sample and PF 0.12 out-of-sample; the fold spread is
  what makes that visible before it reaches production.
* **Calibration, not just loss.** The heads feed an expected-R formula, so a
  miscalibrated 0.9 that should be 0.6 is worse than useless. Brier score and
  a reliability table are reported per head, and a temperature is fitted on a
  held-out slice and saved with the weights.
* **Permutation feature importance** on the OOS block, so "which features
  matter" is measured rather than assumed.
* **An economic score, not only a statistical one.** Every OOS block is
  scored by the realised R of the trades the gate would actually have taken
  (``expected_r`` over the model's own probabilities, net of spread) — sum R,
  avg R, profit factor, max drawdown, trade count. That is the number that
  decides whether this model ships.
* **The feature order is persisted.** v1 fed the net
  ``np.array(list(features_dict.values()))`` and relied on dict ordering
  matching training. Here ``FEATURE_NAMES`` goes into ``*_meta.json`` and the
  strategy refuses to load a model whose feature list disagrees.

Reported win rate is deliberately absent: on this engine ~94% of trades close
at exactly +0.20R, so win rate is a property of the trailing rule, not of the
model. PF / sum R / avg R / drawdown / count are the metrics.
"""
...
from src.strategies.generated.smc_dl_features_v2 import (  # noqa: E402
    FEATURE_NAMES,
    atr,
...
logger = logging.getLogger("train_smc_dl_v2")
...
def build_dataset(
    ...
    features = compute_features_batch(
        ...
        candles.get("H1"),
        candles.get("H4"),
        spread_points=m5["spread_points"].to_numpy(dtype=float),
        point_value=cfg.point_value,
    )
    atr14 = atr(m5, 14)
    label_frame = generate_labels_v2(m5, atr14, labels)
    ...
    spread_price = m5["spread_points"].astype(float) * cfg.point_value
...
def _print_report(meta: dict) -> None:
    ...
    print("\n" + "=" * 72)
    print(f"  {meta['model_name']}  —  {meta['symbol']}")
...
def main() -> int:
    ...
    model_name = args.model_name or (
        ...
        else f"smc_dl_m5_v2_{args.symbol.replace(' ', '_').lower()}"
    )
    cfg = TrainConfig(
        symbol=args.symbol,
    ...
    label_cfg = LabelConfig(target_r=args.target_r) if args.target_r else LabelConfig()
