...
class XauusdSndQmStructureM1:
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | None:
        ...
        if ctx.own_position is not None:
            ...
            # re-fires a fresh multi-leg signal on every candle close where
        ...
        # ── Tiered Take Profit calculation for the 2 target positions (TP1, TP2) ──
        tp_buffer = max(
        ...
        # 1. Locate primary liquidity zone
        zone_target_1 = None
        if demand:
            supply_above = sorted(
