"""XAUUSD S&D + Quasimodo + Market Structure — D1 Position preset (fixed family).

Same core logic as the "fixed" M1/M5/M15/H1 presets but tuned for Daily
entry candles with Weekly (W1) zone detection — a position-trading
approach that captures multi-week/multi-month moves on the largest
structural zones the family supports.
...
Trades XAUUSD using three zone-detection methods and market structure:
  ...
  2. Supply & Demand V2 (engulfing-base departure)
  3. Quasimodo patterns (failure swing → zone)

D1 Position: entries on daily candles, zones detected on W1, base/swing
lookbacks tightened relative to H1 (fewer W1 bars exist per lookback
window than H4 bars), TP buffer widened for Daily's much larger ATR.
Inherits the tp1_target_rr=1.8 fix from xauusd_snd_qm_structure_fixed_h1_v1
(the 1.2 value under-cleared XAUUSD's min_rr=1.5 spread-gate floor).

v1 — D1 variant scaffolded from xauusd_snd_qm_structure_fixed_h1_v1
  (entry_tf H1->D1, zone_tf H4->W1) via the new-strategy skill, 2026-08-05.
...
def _detect_zones_v2(df, atr_series, params):
    ...
    min_engulf_atr = params.get("v2_engulf_min_atr_mult", 1.5)
...
def _detect_quasimodo_zones(df, atr_series, params):
    ...
    lookback = int(params.get("qm_swing_lookback", 6))
    min_swing_atr = params.get("qm_min_swing_atr", 0.8)
...
def _track_zone_on_entry_tf(zone, zone_end_ns, entry_t_ns, entry_highs,
    ...
    """Returns (is_broken, is_price_in_zone_now, is_fresh_touch).
    `is_fresh_touch` is True only on the entry-TF bar price *enters* the
    zone, not on every bar it still sits inside — a retest is an event,
    not a state. Without this gate the strategy re-signalled on every
    candle price lingered in a still-valid zone; see the 2026-08-17 over-trading incident."""
...
# ─────────────────────────────────────────────────────────────────────
# Main strategy class — H1 Short Swing Preset
# ─────────────────────────────────────────────────────────────────────

class XauusdSndQmStructureD1:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="xauusd_snd_qm_structure_fixed_d1",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="D1",
            ...
            params={
                # ── Zone timeframe ──
                "zone_tf_minutes": 10080,
                "entry_tf_minutes": 1440,
                ...
                # ── S&D V2 ──
                "v2_max_base_candles": 6,
                "v2_engulf_min_atr_mult": 1.5,
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | None:
        ...
        for z in all_zones:
            ...
            if is_broken:
                continue
            live_zones.append(z)
            # Gated on fresh_touch (not just is_in_zone) so a zone price
            # still resting inside doesn't re-fire every candle.
