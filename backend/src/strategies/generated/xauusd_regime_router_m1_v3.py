...
`src.strategies.domain.structure_continuation` supplies the first two:
  ...
  * Mode C ("range_reversion") — `_detect_range_reversion()` (inline,
  ...
  DISABLED BY DEFAULT — REAL BACKTEST FINDING (v3)
  ──────────────────────────────────────────────────────────────────────
  A real backtest run against actual XAUUSD history (a separate agent, not
  this file's own synthetic unit-test fixtures) came back with Modes A+B
  strong — trend_continuation PF 5.61, structure_reversal PF 3.28, blended
  PF 4.31 for the two together — while Mode C (range_reversion) ALONE
  scored PF 0.37 (-$2,452), losing in every regime bucket it was tested
  against. Full three-mode blended PF was 1.51, meaning Mode C was actively
  dragging the other two down, not just failing to add value on its own.
  So as of v3, Mode C's code/detector/params stay exactly as designed above
  (nothing deleted, nothing rewritten) but the branch below is gated behind
  `params["enable_range_reversion"]`, which now DEFAULTS TO `False`. Modes
  A and B are unaffected by this — they stay unconditionally on, since
  that's the half of the three-mode design the real backtest actually
  validated. Re-enable Mode C (flip the param to `True`) only after a
  targeted fix and a fresh real backtest confirm it, not on the strength of
  the synthetic-fixture unit tests alone (see
  `test_mode_c_fires_on_band_rejection_when_ranging` — those tests now pass
  `params={"enable_range_reversion": True}` explicitly, precisely so they
  keep exercising the real code path even though it is off by default).
...
`enable_range_reversion` is `True` — off by default per the real-backtest
finding above). No fourth mode, no extra microstructure signals stacked on
top of any of the three — the plan explicitly names `xauusd_iof_scalp_m1`'s
FVG/volume-delta/absorption stack as the cautionary example of that
mistake, and this strategy deliberately keeps to three narrow,
regime-distinct setup types sharing one downstream SL/TP/learner pipeline.
...
class XauusdRegimeRouterM1:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            ...
            params={
                ...
                "range_band_mult": 2.0,
                # v3: OFF by default. A real XAUUSD backtest found Mode C
                # alone at PF 0.37 (-$2,452, losing in every regime bucket)
                # while Modes A+B together ran PF 4.31 — see module
                # docstring's "DISABLED BY DEFAULT" section. The detector
                # and its params above are left fully in place, not
                # deleted, so this is a one-flag re-enable once a fix is
                # validated, not an architecture change.
    ...
    def evaluate(self, ctx):
        ...
        # A and B stay unconditionally on — a real backtest validated them
        # (PF 5.61 / PF 3.28). Mode C is tried last, only while top-level
        # regime reads RANGING, AND only when `enable_range_reversion` is
        # True — OFF by default as of v3, since that same real backtest
        # found Mode C alone at PF 0.37 across every regime bucket (see
        # module docstring's "DISABLED BY DEFAULT" section). The detector
        # itself is untouched; this is purely a call-site gate so
        # re-enabling it later is a one-param flip, not a rewrite.
        mode = None
        setup = None
