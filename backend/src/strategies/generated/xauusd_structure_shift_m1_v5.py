...
candidate is actually traded, keyed by a `{kind}_{direction}` bucket. There
is no live history for this setup family yet, so `prior_logit=0.0`: a cold
bucket falls back to the un-adapted base rule (fire on every valid
candidate, single leg, grid[0] target) rather than guessing — the
learner's own documented failure mode. Every candidate is recorded via
`observe()` regardless of whether the gate passes, or a declined family
could never be re-admitted once it started working again.
...
Once price clears an earlier leg's TP level, `_breakeven_ratchet` requests
`ExitDecision(BREAKEVEN)` on the next `evaluate()` call — the engine routes
that to *every* one of this bot's own open positions sharing its magic
number (see `trade_loop._apply_strategy_exit_decisions`, built explicitly
for a multi-TP-leg bot), so it tightens every remaining leg to entry price
at once. `BREAKEVEN` can only move a stop to entry, never to an arbitrary
level, so a later leg's TP clearing is tracked but has nothing further to
do once the first already moved the stop.
...
class XauusdStructureShiftM1:
    def __init__(self) -> None:
        ...
        # level" and ratchet the remaining legs' SL to breakeven — see
        # `_breakeven_ratchet`. Empty whenever this bot is flat or its
        # last fire was a single leg with nothing to ratchet against.
        # Parallel lists, nearest-TP-first; `_batch_ratcheted[i]` tracks
        # whether leg i's crossing has already been reacted to.
    ...
    def _breakeven_ratchet(self, direction: Direction, close: float) -> ExitDecision | None:
        """Once price clears an earlier (not-yet-runner) leg's TP level,
        request BREAKEVEN — the engine routes an `ExitDecision` to *every*
        one of this bot's own open positions sharing its magic number
        ...
        tightens every remaining leg at once, not just one.

        `BREAKEVEN` can only move a stop to entry price, never to an
        arbitrary level — there is no "move SL to just past TP1" action in
        this codebase's `ExitActionKind`. So a second leg's TP clearing
        later doesn't move the stop any further than the first already
        did; it is tracked and reported (not silently skipped) but is a
        no-op in effect, gated by the engine's own never-loosen invariant.
        """
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | ExitDecision | None:
        ...
        if ctx.own_position is not None:
            ...
            # still gets checked for a breakeven-ratchet opportunity below.
            return self._breakeven_ratchet(ctx.own_position.direction, float(closes[-1]))
        self._batch_direction = None
        self._batch_tp_prices = []
        self._batch_ratcheted = []
        ...
        # Arm the breakeven ratchet for every leg except the runner (the
