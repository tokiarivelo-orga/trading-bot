"""XAUUSD S&D + Quasimodo + Market Structure — M15 Intraday preset.

Same core logic as the M1/M5 presets but tuned for M15 entry candles
with H1 zone detection — a medium-pace intraday style that captures
larger structural moves while resolving within a single session.
...
def _detect_quasimodo_zones(df, atr_series, params):
    ...
    lookback = int(params.get("qm_swing_lookback", 5))
    min_swing_atr = params.get("qm_min_swing_atr", 0.7)
...
# ─────────────────────────────────────────────────────────────────────
# Main strategy class — M15 Intraday Preset
# ─────────────────────────────────────────────────────────────────────

class XauusdSndQmStructureM15:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="xauusd_snd_qm_structure_fixed_m15",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="M15",
            ...
            params={
                ...
                "leg_travel_atr_mult": 0.9,
                "max_base_candles": 8,

                # ── S&D V2 ──
                "v2_max_base_candles": 6,
                "v2_engulf_min_atr_mult": 1.3,

                # ── Quasimodo ──
                "qm_swing_lookback": 5,
                "qm_min_swing_atr": 0.7,
