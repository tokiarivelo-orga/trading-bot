...
picks the take-profit via `best_target`'s MFE-survival optimum, off a grid
starting at 1.8R (clears XAUUSD's `min_rr=1.5` `SpreadGate` floor with
...
class XauusdStructureShiftM5:
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | None:
        ...
        # Note on "tighten the other legs once TP1 hits": that specific
        # mechanic cannot be built here. MT5 never reports back which
        # sibling leg of a multi-leg signal closed — `signal_id` isn't sent
        # to the broker and the 29-char comment cap can't carry a
        # sibling-group id either — so a strategy has no way to know "TP1
        # just hit" from inside evaluate(). Every leg's own stop still
        # ratchets automatically as it moves favorably, via the engine's
        # existing generic secure-base trailing (applies to every open
        # position, not something this file adds or controls).
        return tuple(legs)
