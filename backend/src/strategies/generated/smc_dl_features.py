...
def compute_smc_features(
    candles: dict[str, Any], symbol: str, lookback: int = 20
) -> dict[str, float]:
    """Compute all SMC features for the current bar.
...
def compute_features_batch(
    ...
    """Compute features for every bar in the M5 DataFrame.
    ...
    df["fvg_size"] = np.where(
        df["fvg_bearish"], df["low"].shift(1) - df["high"].shift(-1), df["fvg_size"]
    )
