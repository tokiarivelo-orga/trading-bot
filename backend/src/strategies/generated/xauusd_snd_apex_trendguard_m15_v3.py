"""XAUUSD APEX — S&D + Quasimodo + Trend-Guard + Zone-Respect, M15 entries.

The M15 sibling of `xauusd_snd_apex_trendguard_m1_v5.py`. The algorithm is
...
────────────────────────────────────────────────────────────────────────
    entry / trigger      M15
    zone detection       H1
    higher-TF zones      H4      (confluence + Quasimodo confirmation)
...
candles, not a resample of the entry frame: ~8 days of H1, ~33 days of H4 and ~200 sessions of D1.

────────────────────────────────────────────────────────────────────────
WHY THIS LADDER AND NOT THE OBVIOUS ONE
────────────────────────────────────────────────────────────────────────
The EMA pair drops to 13/34 (from the M1 file's 21/55) purely so the D1
vote is alive from the first replayed bar: a backtest only pre-loads
`run_backtest.HISTORY_BUFFER` = 60 days of context, which is ~43 D1
bars, and a 55-span EMA needs 59. Live this makes no difference (the
engine hands every declared timeframe 200 real bars), but a warm-up hole
would have made the backtest measure a different bot than the one that
trades.
...
# ─────────────────────────────────────────────────────────────────────
# Entry confirmation on the M15 candle
# ─────────────────────────────────────────────────────────────────────


def _confirmation(zone, opens, highs, lows, closes, atr_value, params):
    """Does the last M15 candle agree with the trade?
    ...
    and the right point on that curve differs per bot:
      ...
      "light"  — the candle must merely be closing in the trade's
                 ...
                 zone. Cheap, keeps M15 frequency high. Default.
...
def _overlap_count(zone, others):
    ...
    Confluence: a H1 RBR sitting inside a H4 demand block is a much
...
class XauusdSndApexTrendguardM15:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="xauusd_snd_apex_trendguard_m15",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="M15",
            # Native higher-timeframe candles: the engine gives every entry
            # here its own 200-bar window, so zones are found on real H1
            # and H4 bars instead of a resample of the M15 context.
            confirmation_timeframes=("H1", "H4", "D1"),
            # The engine's own veto is EMA20/50 on M30 only. This
            ...
            params={
                ...
                # inside a H1-sized rectangle for a good while before
                # resolving.
                "max_bars_inside": 20,
                # ── M15 entry confirmation ──
                # Both thresholds are measured against the *zone*-timeframe
                # ATR, so 0.35 of a H1 ATR is a real rejection candle on
                # the M15 chart, not the bare touch the old family entered
                # on.
                "confirm_mode": "light",
                ...
                # 4 M15 bars is the same window here.
                "shock_cooldown_bars": 4,
                # ── Execution ──
        ...
        # new zone-TF bar closes, so it is computed once per H1 bar instead
        # of once per M15 bar. Without this a long M15 replay would redo the
    ...
    def _build_signals(
        ...
        # ── Gate 4: the M15 candle must actually reject the zone ──────
