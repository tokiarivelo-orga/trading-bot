...
v3 — retires two losing BREAKER subtypes entirely: `BREAKER_FVG_BEAR`
  (a broken FVG_BEAR/supply zone that flipped to DEMAND — see
  `_flip_to_breaker`) and `BREAKER_FVG_BULL` (a broken FVG_BULL/demand
  zone flipped to SUPPLY) never produce a `Signal` as of this version.
  Data-mined from this strategy's OWN closed trades for strategy_version
  `xauusd_iof_scalp_m1:v1` (both patterns have zero live history under a
  different version tag — see the v2 note above on why the internal
  `StrategySpec.version` stays 1 across file revisions, for learner/
  trade-tagging continuity), filtered to genuinely independent samples via
  distinct 5-minute open-time buckets (ruling out re-entry-spam artifacts,
  e.g. `BREAKER_RBD`'s 100% win rate on raw count turned out to be 3
  episodes stretched into 24 trades and was correctly left untouched):
    BREAKER_FVG_BEAR (zone_kind=demand, buy):  n=21, 11 independent
      episodes (~1.9 trades/episode), a real 6-day spread (2026-08-13 to
      2026-08-19), win rate 38.1%, total profit -$26.18.
    BREAKER_FVG_BULL (zone_kind=supply, sell): n=21, 11 independent
      episodes, a real 7-day spread (2026-08-12 to 2026-08-19), win rate
      28.6%, total profit -$106.25.
  Both are real, multi-day, multi-episode losers, not noise or a
  re-entry-spam artifact. Re-verified directly against `trades` immediately
  before shipping this version (not taken on faith from an earlier pass):
  BREAKER_FVG_BULL had only 19 trades / -$85.10 at the original
  measurement — 2 more trades have since closed and the pattern has gotten
  *worse*, not better (28.6% WR / -$106.25), so there is no "bad stretch
  that's since turned around" story here.
  IMPLEMENTATION: `_select_candidates` (unchanged) still detects, flips and
  tracks these zones exactly like any other broken zone — lifecycle
  bookkeeping, FVG-confluence distance, and everything else that reads
  `live_zones` stays byte-identical to v2. The exclusion is a single check
  in the candidate loop inside `_decide` (`_is_disabled_breaker_signal()`),
  applied per-candidate right alongside the existing `family_denylist`
  check, so a disabled candidate is simply skipped before `_build_plan`/
  the learner ever see it — no `Signal` is built, no size-multiplier or
  learner-recording path is reached for it. Every other BREAKER subtype
  (`BREAKER_RBR`, `BREAKER_RBD`, `BREAKER_DBD`, `BREAKER_DBR`,
  `BREAKER_SWEEP_LOW`, `BREAKER_SWEEP_HIGH`, `BREAKER_QM*`, ...), plain
  `FVG_BEAR`/`FVG_BULL`, `SWEEP_*`, and every non-breaker family are
  completely untouched — `_is_disabled_breaker_signal()` only ever returns
  True for these two exact pattern+zone_kind combinations.
"""
    ...
    def _decide(
        ...
        *,
        ...
        for zone, life in candidates:
            ...
            if _is_disabled_breaker_signal(zone.get("pattern"), zone["kind"]):
                continue
...
# v3 — retired: BREAKER_FVG_BEAR always flips to ZoneKind.DEMAND (a broken
# FVG_BEAR/supply zone flips sign), BREAKER_FVG_BULL always flips to
# ZoneKind.SUPPLY — deterministic per `_flip_to_breaker` above, given here
# purely defensively. Both are real, multi-day, multi-episode losers on this
# strategy's own closed-trade history — see the module docstring's "v3"
# section for the evidence and the exact re-verified numbers. This function
# is checked once per candidate in `_decide`'s scoring loop, so a match is
# skipped before `_build_plan`/the learner ever see it — zone detection,
# flipping and lifecycle tracking in `_select_candidates` are unaffected.
def _is_disabled_breaker_signal(pattern, zone_kind):
    if pattern == "BREAKER_FVG_BEAR":
        return zone_kind == ZoneKind.DEMAND
    if pattern == "BREAKER_FVG_BULL":
        return zone_kind == ZoneKind.SUPPLY
    return False
