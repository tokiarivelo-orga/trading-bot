...
def generate_labels(
    ...
    for i in range(n):
        ...
        for bar_count, j in enumerate(
            range(i + 1, min(i + 1 + max_holding_bars, n)), start=1
        ):
            h = highs[j]
            low_price = lows[j]
            ...
            if low_price < c_price:
                long_mae = min(long_mae, (low_price - c_price) / c_atr)

            # Short MFE/MAE
            if low_price < c_price:
                short_mfe = max(short_mfe, (c_price - low_price) / c_atr)
            if h > c_price:
                short_mae = min(short_mae, (c_price - h) / c_atr)

            if not hit_long_tp and not hit_long_sl:
                if low_price <= sl_long and h >= tp_long:
                    hit_long_sl = True  # assume SL hits first on huge bar
                elif low_price <= sl_long:
                    hit_long_sl = True
                    long_bars = bar_count
                elif h >= tp_long:
                    hit_long_tp = True
            ...
            if not hit_short_tp and not hit_short_sl:
                if h >= sl_short and low_price <= tp_short:
                    hit_short_sl = True
                elif h >= sl_short:
                    hit_short_sl = True
                    short_bars = bar_count
                elif low_price <= tp_short:
                    hit_short_tp = True
                    short_bars = bar_count
