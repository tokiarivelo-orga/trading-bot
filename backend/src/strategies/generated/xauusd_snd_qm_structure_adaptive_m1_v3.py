...
v2 — evaluate() gained two live-safety gates: (1) a hard `ctx.own_position
  ...
  off.
v3 — adds a bounded, confidence/pattern-tiered SIZING multiplier on top of
  v2's re-entry fix — not a new entry path, and it never opens an extra
  position. Data-mined from closed trades for strategy_version
  xauusd_snd_qm_structure_adaptive_m1:v1 (side=sell, zone_kind=supply):
  when `pattern` is QM_BEAR or QMC and `regime_session` is "overlap"
  (London+NY, ~13:00-16:00 UTC), win rate was 100% (63/63 trades, ~7
  independent episodes by open-time clustering — verified not to be
  re-entry autocorrelation) at confidence>=0.65, and 65%+ specifically at
  confidence>=0.75. Every other pattern/session combination for this
  strategy was meaningfully weaker or a net loser outside the overlap
  session. `evaluate()`'s signal builder now sets `Signal.size_multiplier`
  (new optional field on the `Signal` dataclass, default 1.0 — a bounded
  per-signal RISK-AMOUNT multiplier, not a lot-count multiplier) to
  `sizing_qm_supply_overlap_multiplier` (default 2.0) for a SELL signal
  whose zone is supply, whose pattern is QM_BEAR/QMC, whose session is the
  London+NY overlap (derived from this strategy's own
  session_ny_start_utc/session_london_end_utc params), and whose own
  confidence is >= `sizing_qm_supply_overlap_min_confidence` (default
  0.75) — see `_qm_supply_overlap_size_multiplier()`. Every other signal
  (including every BUY/demand signal — no equivalent finding was validated
  for that side, so it deliberately stays at the default 1.0) is
...
def _track_zone(zone, start_ns, entry_t_ns, highs, lows, closes):
    ...
    return {
        ...
        "touches": touches,
        "in_zone": live_now,
        # True only on the entry-TF bar price *enters* the zone — a
        # retest is an event, not a state. Without this, `_select_
        # candidates` below re-qualified every bar price sat inside an
        # already-touched zone as a fresh candidate; see the 2026-08-17 over-trading incident.
...
def _confluence_score(
    ...
    if trade_dir == 1 and rsi_val < rsi_overbought or trade_dir == -1 and rsi_val > rsi_oversold:
        score += 1
    if session_bucket == 4:
        score += 1
...
def _build_features(
    *,
    ...
    if trade_dir == 1:
        wick = (
            last_bar["low"] < close
            and (min(last_bar["open"], last_bar["close"]) - last_bar["low"])
            or 0.0
        )
    else:
        wick = (
            last_bar["high"] > close
            and (last_bar["high"] - max(last_bar["open"], last_bar["close"]))
            or 0.0
        )

    ix_trend_mom = float(trend_dir * trade_dir) * (
    ...
    ix_height_bb = (zone_height / atr_val) * (
...
class XauusdSndQmStructureAdaptiveM1:
    def __init__(self):
        self.spec = StrategySpec(
            name="xauusd_snd_qm_structure_adaptive_m1",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="M1",
            ...
            params={
                ...
                "tp_band_lo": 0.6,
                "tp_band_hi": 1.6,
                # ── Confidence/pattern-tiered sizing (v3) ──
                # SELL/supply-only: validated on closed trades table for
                # strategy_version xauusd_snd_qm_structure_adaptive_m1:v1
                # (pattern in QM_BEAR/QMC, zone_kind=supply, regime_session=
                # overlap) — 100% win rate (63/63, ~7 independent episodes by
                # open-time clustering) at confidence>=0.65, 65%+ specifically
                # at confidence>=0.75. No equivalent finding exists for the
                # buy/demand side, so this tier is intentionally one-sided.
    ...
    def evaluate(self, ctx: MarketContext):
        ...
        if ctx.own_position is not None:
            ...
            # the 2026-08-17 over-trading incident.
            return None
    ...
    def _select_candidates(self, zones, zone_end_ns, entry_t_ns, highs, lows, closes):
        ...
        for zone in zones:
            ...
            if not life["broken"]:
                live.append(zone)
                # Gated on fresh_touch too: a zone price is still resting
                # inside must not re-qualify as a candidate every bar.
                if life["in_zone"] and life["fresh_touch"]:
            ...
            if flipped_life is None or flipped_life["broken"]:
                continue
            live.append(flipped)
            if flipped_life["in_zone"] and flipped_life["fresh_touch"]:
    ...
    def _decide(
        ...
        *,
        ...
        bb_widths = _bollinger_width(
            zone_closes, int(params.get("bb_period", 20)), float(params.get("bb_std_mult", 2.0))
        )
        ...
        # London+NY overlap window, derived from this strategy's own session
        # params rather than hardcoded — matches session_bucket==4 at the
        # default session_ny_start_utc=13/session_london_end_utc=16. Used
        # only by the v3 sizing tier below.
        ny_start = params.get("session_ny_start_utc", 13)
        london_end = params.get("session_london_end_utc", 16)
        is_overlap_session = ny_start <= hour_utc < london_end

        returns = np.diff(zone_closes) / zone_closes[:-1] if len(zone_closes) > 1 else np.array([])
        hmm_regime = _detect_hmm_regime(
            returns, lookback=int(params.get("range_lookback_bars", 40))
        )
        ...
        *,
        ...
        if learner is not None:
            rr2 = AdaptiveLearner.blend(
                rr2,
                learner.favourable_excursion_r(
                    bucket, float(params.get("tp2_learned_quantile", 0.6))
                ),
            ...
            rr3 = AdaptiveLearner.blend(
                rr3,
                learner.favourable_excursion_r(
                    bucket, float(params.get("tp3_learned_quantile", 0.85))
                ),
                lo_frac=float(params.get("tp_band_lo", 0.6)),
                hi_frac=float(params.get("tp_band_hi", 1.6)),
            )
    ...
    def _build_signals(
        ...
        *,
        ...
        if verdict is not None:
            ...
            learn_note = " p={:.3f}/{:.3f} n={:.0f}{}".format(
                verdict.p_secure,
                threshold,
                verdict.bucket_samples,
                "" if verdict.ready else " (warmup)",
            )
        else:
            learn_note = " learner=off"

        base_reason = "{} [{:.2f},{:.2f}] sl={:.2f} atr={:.2f} touch={} conf={}{}".format(
            zone["family"],
            zone["price_low"],
            zone["price_high"],
            plan["sl_points"],
        ...
        confidence1 = _confidence(verdict, 0.75)
        confidence2 = _confidence(verdict, 0.70)
        confidence3 = _confidence(verdict, 0.65)
        mult1 = _qm_supply_overlap_size_multiplier(zone, is_overlap_session, confidence1, params)
        mult2 = _qm_supply_overlap_size_multiplier(zone, is_overlap_session, confidence2, params)
        mult3 = _qm_supply_overlap_size_multiplier(zone, is_overlap_session, confidence3, params)
        ...
        return (
            Signal(
                ...
                reason=base_reason + " tp1={:.2f} (Scalp)".format(plan["tp1"]),
                zone=annotation,
                pattern=zone.get("pattern"),
            ...
            Signal(
                ...
                reason=base_reason
                + " tp2={:.2f} (Zone rr={:.2f})".format(plan["tp2"], plan["rr2"]),
                zone=annotation,
                pattern=zone.get("pattern"),
                structure=structure_points,
                indicators=readings_tuple,
            ...
            Signal(
                ...
                reason=base_reason
                + " tp3={:.2f} (Runner rr={:.2f})".format(plan["tp3"], plan["rr3"]),
                zone=annotation,
                pattern=zone.get("pattern"),
                structure=structure_points,
                indicators=readings_tuple,
...
def _qm_supply_overlap_size_multiplier(zone, is_overlap_session, confidence, params):
    """Bounded per-signal risk-amount multiplier (v3) — validated ONLY for
    the SELL/supply side: closed trades for strategy_version
    xauusd_snd_qm_structure_adaptive_m1:v1 with pattern in {QM_BEAR, QMC},
    zone_kind=supply and regime_session=overlap (London+NY, ~13:00-16:00
    UTC) had a 100% win rate (63/63 trades, ~7 independent episodes by
    open-time clustering — not re-entry autocorrelation) at confidence>=0.65,
    and 65%+ specifically at confidence>=0.75. No equivalent buy/demand-side
    finding exists, so this deliberately never fires for a BUY signal —
    those keep the default 1.0 multiplier from `Signal`.
    """
    if zone["kind"] != ZoneKind.SUPPLY:
        return 1.0
    if zone.get("pattern") not in ("QM_BEAR", "QMC"):
        return 1.0
    if not is_overlap_session:
        return 1.0
    min_confidence = float(params.get("sizing_qm_supply_overlap_min_confidence", 0.75))
    if confidence < min_confidence:
        return 1.0
    return float(params.get("sizing_qm_supply_overlap_multiplier", 2.0))
