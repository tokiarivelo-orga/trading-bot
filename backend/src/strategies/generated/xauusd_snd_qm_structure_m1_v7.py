...
v5 — evaluate() gained two live-safety gates: (1) a hard `ctx.own_position
  is not None` guard so the bot never opens a second position while one is
  already open, and (2) the zone-touch tracker now returns a `fresh_touch`
  flag and entries require it — previously any candle where price still sat
  inside an already-touched zone re-fired a full 3-leg signal. Root cause of
  the 2026-08-17 over-trading incident (24 positions opened from 4
  consecutive M1 candles at one lingering zone). New version only — the
  prior active version is untouched pending backtest comparison and sign-
  off.
v6 — added a pattern/volatility/session entry filter, reproduced here from
  a validated finding on 3.1k live M1 trades (chronological 70/30 split,
  2026-08-10): a bare S&D/QM signal scores OOS PF 1.02; requiring pattern in
  {RBR, RBD, QM_BULL}, volatility in {low, normal}, and session in {asian,
  london, new_york} - dropping DBD/DBR/DZ_V2/SZ_V2/QM_BEAR, HIGH/EXTREME
  volatility, and the London/New York overlap hour band - held OOS at PF
  1.39 while keeping only ~31% of trades (a real cost-drag saving too, not
  just a quality filter). This is NOT an hour-of-day gate - that was tested
  on the same data and collapsed OOS to PF 0.12 (fitting noise on ~20 hour
  buckets); volatility/session/pattern are coarse, low-cardinality splits
  that held up where hour did not. Volatility and session are recomputed
  here from entry-timeframe candles rather than imported from engine/,
  which generated strategies may not import (CLAUDE.md sandbox rules) - see
  _volatility_ok/_session_ok below, which mirror
  engine/domain/volatility.py / engine/domain/regime.py's classification
  exactly (same ATR period, lookback, percentile bands, and UTC session
  hour boundaries) so the live regime tags this filter is validated against
  and the one it runs on are the same computation.
v7 — additive trend-continuation setup (src.strategies.domain.
  structure_continuation.detect_trend_continuation), layered in only when
  the zone-touch scan above finds nothing (zone-reversal setups keep
  priority whenever both exist). Investigation found today's M1 silence
  after a sharp directional move is entirely gate-driven, not a bug: this
  bot only trades a *retest* of a live zone, and after a strong move price
  can go a long time without coming back to touch one. The new path fires
  on a trend already in motion that just finished a shallow, ATR-bounded
  pullback (a fresh HL in an uptrend / LH in a downtrend, ADX-confirmed
  trending, momentum already resumed beyond the pivot bar) — coverage for
  a market that's moving, not a loosened gate: every existing risk gate
  (EXTREME-volatility circuit breaker via _volatility_ok, the session
  filter via _session_ok, own_position, fresh-touch) still applies to this
  path unchanged. It deliberately does NOT pass through _KEPT_PATTERNS,
  which was fit to zone patterns only (RBR/RBD/QM_BULL) and has no
  bearing on a continuation setup's own TC_UP/TC_DOWN pattern tag. SL
  anchors beyond the pivot using the same sl_zone_buffer_atr_mult/
  sl_min_atr_mult params the zone path already uses; TP follows the same
  fixed tp1/tp2/tp3 R:R ladder the zone path falls back to when it has no
  opposite zone to target (there is never a zone here). VALIDATED, not
  ACTIVE — see the version's own backtest comparison report before any
  activation decision.
v7 (extended, 2026-08-27) — adds a bounded, confidence/pattern/session-
  tiered SIZING multiplier to the zone-touch signal builder only (not the
  v7 trend-continuation path above) - not a new entry path, never opens an
  extra position. Mirrors the sibling xauusd_snd_qm_structure_fixed_m1_v4's
  sizing tier, mined independently from this strategy's own closed trades
  (skill=xauusd_snd_qm_structure_m1), not copied from the sibling's
  numbers. Two qualifying buckets, both reachable through the v6 entry
  filter above (RBD/RBR are in `_KEPT_PATTERNS`, new_york/london are
  allowed sessions):
    - pattern=RBD, zone_kind=supply, session=new_york: n=146 closed
      trades, PF 2.31, win 56.8%, total +$554.
    - pattern=RBR, zone_kind=demand, session=london: n=82, PF 2.85 full
      sample; chronological 70/30 split gives PF 2.37 in-sample (2026-08-07
      to 08-13) rising to PF 3.63 out-of-sample (08-13 to 08-25) — edge
      strengthens rather than decays out-of-sample. 13 independent episodes
      by open-time clustering (>15min gap = new episode). Confidence
      gradient: 48.7% win at 0.65-0.75 confidence (n=117) rising to 74.2%
      at >=0.75 (n=66).
  Both gated at confidence>=0.75 (`sizing_high_conviction_{supply,demand}_
  min_confidence`) with a 1.3x default multiplier
  (`sizing_high_conviction_{supply,demand}_multiplier`) — same
  conservatism as fixed_m1_v4 rather than adaptive_m1's 2.0x. This
  strategy's per-leg confidence is a fixed constant per TP tier (0.75/
  0.70/0.65), so only the TP1/Scalp leg ever qualifies; TP2/TP3 stay at
  the default 1.0. A pattern/session/direction combination NOT tested here
  (e.g. RBR/RBD in asian or new_york/london respectively) also stays at
  the default 1.0 — several of those were checked and are flat-to-negative
  (RBD/supply/asian: PF 0.4 on n=265+235 across both fixed_m1/m1; RBR/
  demand/new_york: PF 0.35-0.48), so this is deliberately narrow rather
  than "every kept pattern in every allowed session."
"""
...
from src.strategies.domain.structure_continuation import detect_trend_continuation
...
# ── Entry filter: pattern + volatility + session (see v6 docstring) ────

_KEPT_PATTERNS = frozenset({"RBR", "RBD", "QM_BULL"})


def _percentile_rank(window: np.ndarray, value: float) -> float:
    """Tie-aware percentile rank, identical to
    engine/domain/volatility.py's _percentile_rank (ties count as half a
    step, so a flat/calm window doesn't pin to the 100th percentile)."""
    if window.size == 0:
        return float("nan")
...
def _volatility_ok(atr_series: pd.Series, lookback: int = 100) -> bool:
    """True when the latest entry-timeframe ATR reading is LOW or NORMAL
    (percentile <= 70 against its own trailing `lookback`-bar history) -
    same bands as engine/domain/volatility.py's latest_volatility_regime
    (low<20, normal 20-70, high>70, extreme>90). Never vetoes on missing
    history - an unknown regime is not evidence of a bad one."""
    values = atr_series.to_numpy()
    valid = values[~np.isnan(values)]
    if valid.size < 2 or np.isnan(values[-1]):
        return True
    ...
    if window.size == 0:
        return True
    return _percentile_rank(window, current) <= 70.0


def _session_ok(hour: int) -> bool:
    """True for asian/london/new_york UTC hours, False for the London/New
    York overlap (12-16) and the 21-22 off-session gap - same boundaries as
    engine/domain/regime.py's session_for() with its default RegimeConfig."""
    if 12 <= hour < 16:
    ...
    return hour >= 22 or hour < 7  # asian (21-22 off-session gap falls through False)


# ── v7 (extended, 2026-08-27) sizing tier: bounded per-signal risk-amount multiplier for two
# high-conviction pattern/session combinations, mined from this strategy's
# own closed trades (skill=xauusd_snd_qm_structure_m1, 2026-08-27). Both
# combinations are reachable through the v6/v7 entry filter above (RBD/RBR
# are in `_KEPT_PATTERNS`, new_york/london are allowed sessions) — neither
# is dead code, unlike the sibling xauusd_snd_qm_structure_fixed_m1_v4's
# QM_BEAR/asian branch.
#
#   - pattern=RBD, zone_kind=supply, session=new_york: n=146 closed trades,
#     PF 2.31, win 56.8%, total +$554. Same combination independently
#     mined and shipped for the sibling fixed_m1 skill (v4) — this is this
#     skill's own data, not copied over.
#   - pattern=RBR, zone_kind=demand, session=london: n=82, PF 2.85 in the
#     full sample; chronological 70/30 split gives PF 2.37 in-sample
#     (2026-08-07 to 08-13) rising to PF 3.63 out-of-sample (08-13 to
#     08-25) — the edge strengthens rather than decays out-of-sample.
#
# Both use the same 0.75-confidence gate and 1.3x default multiplier as
# fixed_m1_v4, for the same reason: this strategy's per-leg confidence is a
# fixed constant per TP tier (0.75/0.70/0.65), so only the TP1/Scalp leg
# ever qualifies.


def _session_label(hour: int) -> str:
    """Which of asian/london/new_york/overlap/off_session `hour` (UTC)
    falls in — same boundaries as `_session_ok` above."""
    if 12 <= hour < 16:
        return "overlap"
    if 7 <= hour < 16:
        return "london"
...
def _high_conviction_supply_size_multiplier(pattern, zone_kind, session_label, confidence, params):
    """Bounded per-signal risk-amount multiplier for a SELL/supply signal
    matching pattern=RBD/session=new_york (see the block comment above).
    Every other pattern/session/direction combination keeps the default
    1.0 multiplier from `Signal`."""
    if zone_kind != ZoneKind.SUPPLY:
        return 1.0
    if not (pattern == "RBD" and session_label == "new_york"):
        return 1.0
    min_confidence = float(params.get("sizing_high_conviction_supply_min_confidence", 0.75))
    if confidence < min_confidence:
        return 1.0
    return float(params.get("sizing_high_conviction_supply_multiplier", 1.3))


def _high_conviction_demand_size_multiplier(pattern, zone_kind, session_label, confidence, params):
    """Bounded per-signal risk-amount multiplier for a BUY/demand signal
    matching pattern=RBR/session=london (see the block comment above).
    Every other pattern/session/direction combination keeps the default
    1.0 multiplier from `Signal`."""
    if zone_kind != ZoneKind.DEMAND:
        return 1.0
    if not (pattern == "RBR" and session_label == "london"):
        return 1.0
    min_confidence = float(params.get("sizing_high_conviction_demand_min_confidence", 0.75))
    if confidence < min_confidence:
        return 1.0
    return float(params.get("sizing_high_conviction_demand_multiplier", 1.3))
...
def _detect_quasimodo_zones(df, atr_series, params):
    ...
    for idx in range(len(swings) - 3):
        ...
        # Pattern: the second low goes below the first (lower low), and the
        # subsequent high fails to reach the previous high (lower high, or
        # just fails) → demand zone at the lower low
        if (
            s1[2] == "low"
            and s2[2] == "high"
            and s3[2] == "low"
        ...
        # Pattern: the second high goes above the first (higher high), and
        # the subsequent low fails to reach the previous low (higher low)
        # → supply zone at the higher high
        if (
            s1[2] == "high"
            and s2[2] == "low"
            and s3[2] == "high"
...
def _track_zone_on_entry_tf(zone, zone_end_ns, entry_t_ns, entry_highs, entry_lows, entry_closes):
    ...
    price *enters* the zone, not on every bar it still sits inside — a
    retest is an event, not a state. Without this gate the strategy
    re-signalled on every candle price lingered in a still-valid zone;
    see the 2026-08-17 over-trading incident."""
    ...
    # Fresh touch: was the *previous* entry-TF bar (since this zone
    # became valid) NOT in the zone? The first bar of the zone's life
    # always counts as fresh.
    prev_idx = last_idx - 1
    if prev_idx < start:
        was_in_zone = False
    elif demand:
        was_in_zone = entry_lows[prev_idx] <= zone["price_high"]
    else:
        was_in_zone = entry_highs[prev_idx] >= zone["price_low"]
    is_fresh_touch = bool(in_zone and not was_in_zone)

    return is_broken, in_zone, is_fresh_touch
...
class XauusdSndQmStructureM1:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            ...
            params={
                ...
                "tp_buffer_atr_mult": 0.3,  # Dynamic ATR front-run distance
                # before liquidity rejection
                # ── Break Even ──
                "be_buffer_points": 10,
                # ── Trend continuation (v7, additive — see module docstring):
                # fires only when the zone-touch scan above finds nothing.
                # Key names match src.strategies.domain.structure_continuation's
                # own `params` lookups exactly (unprefixed) so this dict can be
                # handed straight through to detect_trend_continuation(). ──
                "pivot_bars": 2,  # fractal half-window for swing pivots
                "trend_adx_period": 14,
                "trend_adx_threshold": 25.0,  # ADX floor to call the market TRENDING
                "trend_ema_fast_span": 12,  # direction: EMA(fast) vs EMA(slow) sign
                "trend_ema_slow_span": 34,
                "min_pullback_atr": 0.5,  # pullback floor, in ATR — excludes noise
                "max_pullback_atr": 2.5,  # pullback ceiling — deeper looks like a
                # reversal (the zone detectors' territory), not a continuation
                "momentum_confirm_atr_mult": 0.1,  # close must clear the pivot bar's own
                # high/low by this many ATR before momentum counts as "resumed"
                # ── Position management ──
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | None:
        ...
        if ctx.own_position is not None:
            # Only one position at a time. Without this guard, evaluate()
            # re-fires a fresh 3-leg signal on every candle close where
            # price still sits inside an already-traded zone — see
            # the 2026-08-17 over-trading incident for what that costs live.
            return None
        ...
        for z in all_zones:
            ...
            is_broken, is_in_zone, is_fresh_touch = _track_zone_on_entry_tf(
            ...
            # Entry: price *freshly* touches/enters the zone, zone still
            # valid. Gated on fresh_touch (not just is_in_zone) so a zone
            # price is still resting inside doesn't re-fire every candle.
            if is_in_zone and is_fresh_touch and candidate is None:
                candidate = z

        if candidate is None:
            # v7: no zone-touch setup — try the additive trend-continuation
            # path before giving up. Reuses the zone-timeframe OHLC/ATR
            # already computed above rather than refetching anything.
            continuation = detect_trend_continuation(
                highs=zone_frame["high"].to_numpy(),
                lows=zone_frame["low"].to_numpy(),
                closes=zone_frame["close"].to_numpy(),
                times=list(zone_frame["time"]),
            ...
            # Same volatility/session gates the zone-touch path runs below
            # (see v6 docstring) — deliberately NOT _KEPT_PATTERNS, which
            # was fit to zone patterns only and has no bearing here.
            entry_atr_series = _atr(df, 14)
            if not _volatility_ok(entry_atr_series):
                return None
            if not _session_ok(int(pd.Timestamp(t).hour)):
                return None

            is_buy = continuation.direction == Direction.BUY
            tc_pattern = "TC_UP" if is_buy else "TC_DOWN"

            # SL: beyond the pivot by the same ATR buffer the zone path
            # uses (sl_zone_buffer_atr_mult) — no zone-height component,
            # there is no zone here.
            buffer = atr_val * float(params.get("sl_zone_buffer_atr_mult", 0.15))
            if is_buy:
                sl_points = (close - continuation.pivot_price) + buffer
            else:
                sl_points = (continuation.pivot_price - close) + buffer
            ...
            # TP: the same fixed tp1/tp2/tp3 R:R ladder the zone path falls
            # back to when it has no opposite zone to target — there is
            # never a zone target for a continuation setup.
            tp1_points = risk_price * float(params.get("tp1_target_rr", 1.8))
            ...
            base_reason = (
                f"TREND_CONTINUATION/{tc_pattern} pivot={continuation.pivot_price:.2f} "
                f"pullback={continuation.pullback_depth_atr:.2f}atr "
                f"adx={continuation.trend_strength:.1f} sl={sl_points:.2f}"
            )
            ...
            sig_tp1 = Signal(
                ...
                zone=None,
                pattern=tc_pattern,
                structure=continuation.structure_points,
            )
            sig_tp2 = Signal(
                direction=direction,
                sl_points=float(sl_points),
        ...
        # Pattern/volatility/session filter (see v6 docstring) - deliberately
        # NOT an hour-of-day gate, which overfit catastrophically on this
        # exact data (OOS PF 0.12).
        if candidate.get("pattern") not in _KEPT_PATTERNS:
            return None
        entry_atr_series = _atr(df, 14)
        if not _volatility_ok(entry_atr_series):
            return None
        if not _session_ok(int(pd.Timestamp(t).hour)):
            return None
        session_label = _session_label(int(pd.Timestamp(t).hour))

        demand = candidate["kind"] == ZoneKind.DEMAND
        direction = Direction.BUY if demand else Direction.SELL

        # ── Optimized Stop Loss (anchored to execution price + dynamic buffer + ATR floor) ──
        ...
        if demand:
            supply_above = sorted(
                [z for z in opposite_zones if z["price_low"] > close],
        ...
        # 2. Determine TP1 (Quick Scalp / Momentum target): short distance to
        # pay spread and trigger breakeven
        ...
        if zone_target_1 is not None and zone_target_1 > tp1_points:
            ...
            tp2_points = max(
                tp1_points + risk_price * 1.0,
                risk_price * float(params.get("tp2_target_rr", 2.5)),
            )
        ...
        if zone_target_2 is not None and zone_target_2 > tp2_points:
            ...
            tp3_points = max(
                tp2_points + risk_price * 1.5,
                risk_price * float(params.get("tp3_target_rr", 4.0)),
            )
        ...
        base_reason = (
            ...
            f"[{candidate['price_low']:.2f},{candidate['price_high']:.2f}] sl={sl_points:.2f}"
        )

        # v7 (extended, 2026-08-27) sizing tier (see module comment above / _high_conviction_supply_
        # size_multiplier + _high_conviction_demand_size_multiplier): each TP
        # leg's own fixed confidence decides whether it qualifies. Only the
        # TP1/Scalp leg (confidence 0.75) can ever clear the default 0.75
        # threshold below. The two functions gate on opposite `zone_kind`s
        # so only one of them can ever return >1.0 for a given candidate —
        # max() just picks whichever (if either) qualifies.
        mult1 = max(
            _high_conviction_supply_size_multiplier(
                candidate.get("pattern"), candidate["kind"], session_label, 0.75, params
            ),
        ...
        mult2 = max(
            _high_conviction_supply_size_multiplier(
                candidate.get("pattern"), candidate["kind"], session_label, 0.70, params
            ),
            _high_conviction_demand_size_multiplier(
                candidate.get("pattern"), candidate["kind"], session_label, 0.70, params
            ),
        ...
        mult3 = max(
            _high_conviction_supply_size_multiplier(
                candidate.get("pattern"), candidate["kind"], session_label, 0.65, params
            ),
            _high_conviction_demand_size_multiplier(
                candidate.get("pattern"), candidate["kind"], session_label, 0.65, params
            ),
