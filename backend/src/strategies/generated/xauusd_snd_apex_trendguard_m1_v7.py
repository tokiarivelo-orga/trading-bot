...
def _m1_reversal_streak(highs, lows, want, window):
    """Trailing run-length, ending at the latest M1 bar, of bars whose
    high AND low both moved against `want` versus the previous bar.

    A streak spanning the whole `window` means short-term M1 momentum has
    already flipped against the trade in the minutes right before entry —
    the exact pattern behind a subsequent review's 10-for-10 loss sample:
    every M1 confirmation candle fired 1-3 minutes after the M5 series'
    local high, with price already rolling over into the reversal that
    then hit the stop. `_confirmation` alone only checks that the *current*
    candle closes the right way; it cannot see that the two candles before
    it were already printing lower highs and lower lows against a buy.
    """
    n = len(highs)
    if n < 2 or window < 2:
        return 0
    span = min(window, n)
    recent_highs = highs[n - span :]
    recent_lows = lows[n - span :]
    streak = 0
    for i in range(1, span):
        if want > 0:
            against = recent_highs[i] < recent_highs[i - 1] and recent_lows[i] < recent_lows[i - 1]
        else:
            against = recent_highs[i] > recent_highs[i - 1] and recent_lows[i] > recent_lows[i - 1]
...
class XauusdSndApexTrendguardM1:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            ...
            params={
                ...
                # M1 momentum freshness (Gate 4b). A follow-up review found
                # 10/10 losing buys fired within 1-3 minutes of the M5
                # series' local high, with the last 2-3 M1 closes already
                # printing lower highs and lower lows against the trade —
                # `_confirmation`'s single-candle check missed this. Window
                # of 3 bars = 2 comparisons; a full-window streak vetoes.
                "m1_freshness_bars": 3,
                # ── Stop loss ──
    ...
    def _build_signals(
        ...
        # ── Gate 4b: M1 short-term momentum freshness ─────────────────
        # A follow-up review found 10/10 losing buys fired 1-3 minutes
        # after the M5 series' local high, with the last 2-3 M1 closes
        # already printing lower highs and lower lows against the trade —
        # Gate 4 only checks the *current* candle, so it missed this.
        freshness_window = int(params["m1_freshness_bars"])
        reversal_streak = _m1_reversal_streak(highs, lows, want, freshness_window)
        streak_ceiling = freshness_window - 1
        if streak_ceiling >= 1 and reversal_streak >= streak_ceiling:
            self._reject("m1_momentum_reversed")
            return None
        ...
        readings = (
            ...
            IndicatorReading(
                name="m1_reversal_streak",
                value=float(reversal_streak),
                threshold=float(streak_ceiling),
                comparison="<",
                passed=True,
            ),
        )
