...
def _htf_trend_bias(structure: list) -> str:
    """'down' if the latest swing high is a Lower High and the latest swing
    low is a Lower Low (active downtrend), 'up' if both are Higher
    High/Higher Low, else 'neutral'. Used to veto counter-trend zone
    entries — see v6 module docstring note."""
    highs = [s for s in structure if s["label"] in (StructureLabel.HH, StructureLabel.LH)]
    lows = [s for s in structure if s["label"] in (StructureLabel.LL, StructureLabel.HL)]
    if not highs or not lows:
        return "neutral"
    last_high = highs[-1]
    last_low = lows[-1]
    if last_high["label"] == StructureLabel.LH and last_low["label"] == StructureLabel.LL:
        return "down"
    if last_high["label"] == StructureLabel.HH and last_low["label"] == StructureLabel.HL:
        return "up"
    return "neutral"
...
# ─────────────────────────────────────────────────────────────────────
# Main strategy class — M5 Scalping Preset
# ─────────────────────────────────────────────────────────────────────


class XauusdSndQmStructureM5:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="xauusd_snd_qm_structure_m5",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="M5",
            ...
            params={
                ...
                "max_base_candles": 6,
                # ── S&D V2 ──
                "v2_max_base_candles": 5,
                "v2_engulf_min_atr_mult": 1.2,
                # ── Quasimodo ──
                "qm_swing_lookback": 4,
                "qm_min_swing_atr": 0.6,
                # ── Market structure ──
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | None:
        ...
        # ── HTF market structure (reused for the trend veto below and
        # for the annotation on the returned signals) ──
        ...
        # ── HTF trend veto: skip a demand-zone buy fired into an active
        # M15 downtrend, and symmetrically skip a supply-zone sell fired
        # into an active uptrend. Added after a review found every loss
        # in a 10-trade sample was a counter-trend buy — see v6 note. ──
        htf_bias = _htf_trend_bias(structure)
        if (demand and htf_bias == "down") or (not demand and htf_bias == "up"):
            return None
        ...
        # ── Market Structure for annotation (already computed above) ──
