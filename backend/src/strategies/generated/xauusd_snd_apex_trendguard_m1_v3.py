...
def _trend_state(ctx, params):
    ...
    for timeframe in ("M5", "M15", "H1"):
        ...
        score += vote * float(weights.get(timeframe, 1.0))
    return score, votes
...
def _confirmation(zone, opens, highs, lows, closes, atr_value, params):
    ...
    and the right point on that curve differs per bot:
      ...
      "strict" — additionally demands a real rejection candle: a body of
                 `confirm_body_atr_mult` x ATR and a wick back into the
                 zone worth `confirm_wick_frac` of the bar's range.
    ...
    if body < min_body:
        return False, f"body {body:.2f} < {min_body:.2f}"

    wick_frac = wick / bar_range
    min_wick = float(params["confirm_wick_frac"])
    if wick_frac < min_wick:
        return False, f"wick {wick_frac:.2f} < {min_wick:.2f}"
