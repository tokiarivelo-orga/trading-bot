...
def _momentum_invalidated(zone, zone_highs, zone_lows, zone_closes, zone_atr_values, params):
    """True if price has already run through the zone against the trade
    direction with an accelerating, above-ATR range over the last few
    zone-timeframe candles — i.e. this is a breakout, not a fadeable zone,
    regardless of any confirmation that fired earlier."""
    lookback = int(params.get("momentum_invalidation_lookback", 3))
    n = len(zone_closes)
    if n < lookback:
        return False
    expand_mult = float(params.get("momentum_invalidation_atr_mult", 1.2))
    demand = zone["kind"] == ZoneKind.DEMAND
    idxs = range(n - lookback, n)
    beyond = 0
    ranges_atr = []
    for i in idxs:
        atr_at = zone_atr_values[i] if i < len(zone_atr_values) else np.nan
    ...
    if beyond < 2:
        return False
    return ranges_atr[-1] >= expand_mult and ranges_atr[-1] >= ranges_atr[0]
...
class XauusdSndQmStructureAdaptiveM1:
    def __init__(self):
        self.spec = StrategySpec(
            ...
            params={
                ...
                # ── Momentum breakout invalidation ──
                "momentum_invalidation_lookback": 3,
                "momentum_invalidation_atr_mult": 1.2,
                # ── Stop loss ──
    ...
    def _decide(
        ...
        *,
        ...
        zone_atr_values = atr_series.to_numpy()
