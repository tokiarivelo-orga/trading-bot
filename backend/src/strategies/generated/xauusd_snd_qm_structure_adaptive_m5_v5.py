...
def _active_countertrend_impulse(closes, trade_dir, lookback):
    """True if the last `lookback` closes are moving consecutively against trade_dir."""
    if len(closes) < lookback:
        return False
    diffs = np.diff(closes[-lookback:])
    if len(diffs) == 0:
        return False
    if trade_dir == -1:
...
class XauusdSndQmStructureAdaptiveM1:
    def __init__(self):
        self.spec = StrategySpec(
            ...
            params={
                ...
                # Countertrend momentum veto — hard-vetoes entries against an
                # active, momentum-confirmed impulse regardless of confluence
                # score or learner history (see review 2026-09-09).
                "momentum_veto_bars": 3,
                "momentum_veto_htf_swing_lookback": 6,
                # Spread quality
    ...
    def _htf_state(self, ctx, params):
        ...
        swing_window = int(params.get("momentum_veto_htf_swing_lookback", 6))
        fresh_high = False
        fresh_low = False
        if len(closes) > swing_window:
            recent_window = closes[-(swing_window + 1) :]
            fresh_high = bool(recent_window[-1] >= recent_window.max())
            fresh_low = bool(recent_window[-1] <= recent_window.min())
        return {
            "dir": direction,
    ...
    def _decide(
        ...
        *,
        ...
        entry_closes_m5 = df["close"].to_numpy()
        momentum_veto_bars = int(params.get("momentum_veto_bars", 3))
        ...
        for zone, life in candidates:
            ...
            # Hard veto: never enter countertrend into an active, momentum-
            # confirmed impulse — no confluence score or learner history can
            # override this (see review 2026-09-09: 6/10 sells shorted a
            # supply zone straight into a confirmed bullish breakout).
            if _active_countertrend_impulse(entry_closes_m5, trade_dir, momentum_veto_bars):
                continue
            htf_fresh_against = (
                self._htf["fresh_high"] if trade_dir == -1 else self._htf["fresh_low"]
            )
            if htf_fresh_against:
                continue
