...
def _bars_since_shock(opens, highs, lows, closes, lookback, mult):
    """How many entry-TF bars ago the last abnormal range expansion — or a
    session/weekend gap — was, or None if there hasn't been one in the
    visible window.
    ...
    Original form only ever compared a bar's own high-low range to the
    trailing median. A session-open gap (2026-09-08: last pre-break M5 at
    2026-09-07T18:25 closed 4406.64, next bar at 2026-09-08T00:00 opened
    4423.16 — a ~$16 jump) shows up as no single abnormal-range candle at
    all, so it never tripped this guard, and the trend gate's H1/H4 read
    stayed stale from before the gap: four trades opened straight into it
    at 01:15-01:20 UTC and were stopped out near full SL. The companion
    check below compares `abs(open[i] - close[i-1])` against the same
    `mult * trailing_median` threshold used for intrabar range, so a gap
    trips the identical `shock_cooldown_bars` pause a range shock would.
    """
    n = len(highs)
    if n < lookback + 2:
        return None
    ...
    for back in range(n - 1, max(-1, n - 1 - lookback), -1):
        if float(ranges[back]) >= threshold:
            return (n - 1) - back
        if back > 0 and abs(float(opens[back]) - float(closes[back - 1])) >= threshold:
            return (n - 1) - back
    return None
...
# ─────────────────────────────────────────────────────────────────────
# Entry confirmation on the M5 candle
# ─────────────────────────────────────────────────────────────────────


def _confirmation(zone, opens, highs, lows, closes, atr_value, params):
    """Does the last M5 candle agree with the trade?
    ...
    and the right point on that curve differs per bot:
      ...
      "light"  — the candle must merely be closing in the trade's
                 ...
                 zone. Cheap, keeps M5 frequency high. Default.
...
def _overlap_count(zone, others):
    ...
    Confluence: a M15 RBR sitting inside a H1 demand block is a much
...
class XauusdSndApexTrendguardM5:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="xauusd_snd_apex_trendguard_m5",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="M5",
            # Native higher-timeframe candles: the engine gives every entry
            # here its own 200-bar window, so zones are found on real M15
            # and H1 bars instead of a resample of the M5 context.
            confirmation_timeframes=("M15", "H1", "H4"),
            # The engine's own veto is EMA20/50 on M15 only. This
            ...
            params={
                ...
                "trend_ema_slow": 55,
                "trend_slope_bars": 3,
                "trend_weights": {"M15": 1.0, "H1": 1.5, "H4": 2.0},
                ...
                # inside a M15-sized rectangle for a good while before
                # resolving.
                "max_bars_inside": 20,
                # ── M5 entry confirmation ──
                # Both thresholds are measured against the *zone*-timeframe
                # ATR, so 0.35 of a M15 ATR is a real rejection candle on
                # the M5 chart, not the bare touch the old family entered
                # on.
                "confirm_mode": "light",
                ...
                # 6 M5 bars is the same window here.
        ...
        # new zone-TF bar closes, so it is computed once per M15 bar instead
        # of once per M5 bar. Without this a long M5 replay would redo the
    ...
    def evaluate(self, ctx):
        ...
        # a release, a rollover gap, or a session-open jump, no amount of
        # zone quality makes an M5 entry a good idea.
    ...
    def _build_signals(
        ...
        # ── Gate 4: the M5 candle must actually reject the zone ───────
