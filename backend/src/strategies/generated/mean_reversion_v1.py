"""Price-structure mean-reversion strategy: fades HH/LL exhaustion.

Built after `trend_structure_v1`/`v2` (which buy a fresh swing high / sell a
...
1.6-1.7, ~43% win rate) across all three symbols and every tested swing
window, which is why this strategy trades the opposite direction of
`trend_structure_*`: on this data, a new M5 extreme tends to mark local
exhaustion rather than continuation.

Same fractal swing-pivot detection as `trend_structure_v1`, same TP:SL
(2.2) and same SL-distance convention (the leg into the fresh extreme), only
...
PIVOT_WING = 8  # bars required on each side of a candidate to confirm a swing
MIN_HISTORY = 50  # enough bars for at least 3 alternating swing pivots at this wing
...
class MeanReversionV1:
    ...
    def evaluate(self, ctx: MarketContext) -> Signal | None:
        ...
        if prior_kind != last_kind:
            return None
        _, sl_reference, _ = swings[-2]  # opposite-kind pivot between them: SL distance anchor

        if last_kind == "high" and last_price > prior_price:
            # Fresh HH: fade it — sell, betting on exhaustion/reversion.
            direction, label = Direction.SELL, StructureLabel.HH
        elif last_kind == "low" and last_price < prior_price:
            # Fresh LL: fade it — buy, betting on exhaustion/reversion.
            direction, label = Direction.BUY, StructureLabel.LL
        else:
            return None
        ...
        return Signal(
            ...
            confidence=0.55,
            reason=(
                f"fading {label.value} at {last_price:.5f} (bar {last_index}) vs prior swing "
