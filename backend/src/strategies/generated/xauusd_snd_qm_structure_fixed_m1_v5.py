...
def _weekend_close_ok(ts: pd.Timestamp, close_hour_utc: int, block_hours: float) -> bool:
    """False during the last `block_hours` before the weekly Friday market
    close (UTC) - a position opened there gets carried into the weekend
    and a stop-loss cannot execute through a gap if price reopens Sunday
    beyond it. Added after all 3 losses in the last-10-trades review
    (2026-09-07) were the same BUY opened 2026-09-04 20:07 UTC, held over
    the weekend, and gapped straight through its SL on Sunday reopen. Only
    Friday is gated - Saturday and pre-reopen Sunday already have no
    candle flow reaching evaluate(), so no separate check is needed there.
    """
    dow = int(ts.dayofweek)  # Monday=0 ... Sunday=6
    if dow != 4:  # Friday is the only day a new position can be carried
        return True  # straight into a weekend close
    hour = float(ts.hour) + float(ts.minute) / 60.0
    return hour < (close_hour_utc - block_hours)
...
class XauusdSndQmStructureM1:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            ...
            params={
                ...
                # ── Weekend close guard (see _weekend_close_ok) ──
                "weekly_close_hour_utc": 22,  # UTC hour the weekly market
                # closes on Friday. All 3 losses reviewed 2026-09-07 opened
                # 20:07 UTC Friday and gapped through their SL over the
                # weekend; broker reopened 22:10 UTC Sunday, consistent
                # with a ~22:00 UTC close.
                "weekend_block_hours_before_close": 2,  # No new entries in
                # this window before the Friday close - matches the
                # reviewer's "roughly 1-2 hours" finding with headroom.
                # ── Take Profit ──
                "tp_buffer_points": 20,
                "point_value": 0.01,
    ...
    def evaluate(self, ctx: MarketContext) -> tuple[Signal, ...] | None:
        ...
        if not _volatility_ok(entry_atr_series):
            return None
        ts_full = pd.Timestamp(t)
        if not _session_ok(int(ts_full.hour)):
            return None
        if not _weekend_close_ok(
            ts_full,
            int(params.get("weekly_close_hour_utc", 22)),
            float(params.get("weekend_block_hours_before_close", 2)),
        ):
            return None
