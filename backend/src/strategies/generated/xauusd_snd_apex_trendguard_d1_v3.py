"""XAUUSD APEX — S&D + Quasimodo + Trend-Guard + Zone-Respect, D1 entries.

The D1 sibling of `xauusd_snd_apex_trendguard_m1_v5.py`. The algorithm is
...
────────────────────────────────────────────────────────────────────────
    entry / trigger      D1
    zone detection       D1
    higher-TF zones      W1      (confluence + Quasimodo confirmation)
    trend votes          H4 (w 1.0), D1 (w 2.0)
...
candles, not a resample of the entry frame: ~33 days of H4 and ~4 years of W1.

────────────────────────────────────────────────────────────────────────
WHY THIS LADDER AND NOT THE OBVIOUS ONE
────────────────────────────────────────────────────────────────────────
Two departures from the finer siblings, both forced by being at the top
of the ladder:
  * `zone_timeframe` equals the entry timeframe. Every other sibling
    draws zones one rung up and triggers on a finer candle; here there
    is no finer candle above D1, so the daily bar both forms the zone
    and carries the rejection close. There is still no look-ahead — a
    zone is only usable from the bar after its leg-out completes, which
    `_zone_lifecycle` enforces by timestamp.
...
# ─────────────────────────────────────────────────────────────────────
# Entry confirmation on the D1 candle
# ─────────────────────────────────────────────────────────────────────


def _confirmation(zone, opens, highs, lows, closes, atr_value, params):
    """Does the last D1 candle agree with the trade?
    ...
    and the right point on that curve differs per bot:
      ...
      "light"  — the candle must merely be closing in the trade's
                 ...
                 zone. Cheap, keeps D1 frequency high. Default.
...
def _overlap_count(zone, others):
    ...
    Confluence: a D1 RBR sitting inside a W1 demand block is a much
...
class XauusdSndApexTrendguardD1:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="xauusd_snd_apex_trendguard_d1",
            version=1,
            symbols=("XAUUSD",),
            entry_timeframe="D1",
            # Native higher-timeframe candles: the engine gives every entry
            # here its own 200-bar window, so zones are found on real D1
            # and W1 bars instead of a resample of the D1 context.
            confirmation_timeframes=("H4", "W1"),
            # The engine's own veto is EMA20/50 on W1 only. This
            ...
            params={
                ...
                "trend_ema_slow": 34,
                "trend_slope_bars": 3,
                "trend_weights": {"H4": 1.0, "D1": 2.0},
                # Of a possible +/-3.0. Requires real multi-timeframe
                ...
                # inside a D1-sized rectangle for a good while before
                # resolving.
                "max_bars_inside": 20,
                # ── D1 entry confirmation ──
                # Both thresholds are measured against the *zone*-timeframe
                # ATR, so 0.35 of a D1 ATR is a real rejection candle on
                # the D1 chart, not the bare touch the old family entered
                # on.
                "confirm_mode": "light",
                ...
                # 1 D1 bars is the same window here.
        ...
        # new zone-TF bar closes, so it is computed once per D1 bar instead
        # of once per D1 bar. Without this a long D1 replay would redo the
    ...
    def _build_signals(
        ...
        # ── Gate 4: the D1 candle must actually reject the zone ───────
