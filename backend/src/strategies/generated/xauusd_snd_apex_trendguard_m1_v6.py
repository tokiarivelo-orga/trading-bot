"""XAUUSD APEX — S&D + Quasimodo + Trend-Guard + Zone-Respect, M1 entries.

A ground-up rebuild of the `xauusd_snd_qm_structure_*` family, driven by a
forensic post-mortem of the 2026-08-05 session (608 XAUUSD positions,
41.2% win rate, profit factor 0.73, -$889.58 on the day).

────────────────────────────────────────────────────────────────────────
WHAT THE 2026-08-05 DATA SAID  (every default below traces to a number)
────────────────────────────────────────────────────────────────────────
Gold ran 4072 -> 4200 (+128) in one session. The old bots had no trend
filter at all (their own docstring: "no higher-timeframe trend filter, no
R:R filter — every valid signal trades"), so they sold every supply zone
the rally went through.

  1. DIRECTION.  sells 306 trades / 20.9% WR / -$1154.84
                 buys  301 trades / 61.8% WR / +$265.26
     Reversal-at-supply patterns were wiped out: RBD 90 trades 0.0% WR
     -$777.83, SZ_V2 89 trades 4.5% WR -$796.12, QM_BEAR 37 trades 0.0%
     WR -$225.59.  The one bearish pattern that worked was DBD
     (continuation inside a down-leg): 66.7% WR, +$644.70.
     -> Fix: `_trend_state` + `_zone_respect_index` gate direction.

  2. STRUCTURE WAS COMPUTED BUT NEVER ENFORCED.  The old code attached
     HH/HL/LH/LL points to every signal and used them only for chart
     annotation.  Sliced by that same annotation:
         bias=up   344 trades / 63.7% WR / +$709.81
         bias=down 188 trades /  5.9% WR / -$1580.08
     -> Fix: structure is now a hard entry gate, not decoration.

  3. THE SAME DEAD ZONE WAS TRADED OVER AND OVER.  Zone invalidation only
     fired when a candle *closed* through the far side, while entry fired
     whenever price merely touched the near side.  40 distinct zones
     produced 302 signals / 607 positions.  Supply [4151.86,4158.11] was
     sold 18 separate times for -$462.  Zones re-fired 2-6 times scored
     ~7% WR / -$968.
     -> Fix: `_zone_lifecycle` counts discrete retest episodes and marks a
        zone dead once price has eaten through it; only episode #1 trades.

  4. ENTRY ON BARE TOUCH.  The old rule was `high >= zone_low` — i.e. buy
     the instant price arrives, including when it is arriving at speed on
     its way through.  -> Fix: `_confirmation` requires a real M1
     rejection/engulf close back out of the zone.

  5. EXITS GAVE BACK THE EDGE.  Of 250 non-stopped trades, 200 (80%) were
     cut before target, capturing a median of only 13% of their TP
     distance.  R-budget for the day:
         stop-outs  357 x -1.04R = -371.6R
         scratched  200 x +0.75R = +149.5R
         real TPs    50 x +2.54R = +126.8R   -> net -95.4R
     The engine's PositionManager rule 3 snaps the stop to break-even
     whenever price comes within 0.5R of an opposing base — and the old
     TP2 was set *at* the opposing zone, so it was structurally
     guaranteed to arm that rule before filling.
     -> Fix: `tp_frontrun_r_mult` places every target at least 0.6R short
        of the opposing zone, outside rule 3's trigger band, and the
        ladder drops from 3 legs to 2 (the old middle leg was the single
        worst bucket of the day at -$514.25).

  6. ZONE QUALITY.  height 3-4 USD: 53.7% WR / +$366 | height 6-9 USD:
     10.4% WR / -$1074.  Entry depth 0.50-0.75 into the zone: 19.0% WR /
     -$529.89.  -> Fix: `zone_height_atr_min/max` and `max_entry_depth`.

  7. STOP SIZING.  SL 4-5 USD: 22.5% WR / -$516.14; SL 5-7 USD: -$277.94.
     Oversized stops came from oversized zones.  -> Fix: `sl_max_atr_mult`
     caps the stop and the trade is skipped rather than widened past it.

────────────────────────────────────────────────────────────────────────
STRUCTURAL UPGRADE: NATIVE HIGHER-TIMEFRAME CANDLES
────────────────────────────────────────────────────────────────────────
The old family resampled its 200-bar M1 context into M5, leaving ~40 M5
bars, of which ATR(14) consumed 14 — roughly 26 usable bars to find zones
in.  That is why its zones were noisy.  The engine hands each declared
`confirmation_timeframes` entry its own full 200-bar window
(`trade_loop.DEFAULT_CONTEXT_BARS`), so this strategy asks for M5, M15 and
H1 directly and gets 200 native bars of each: ~16h of M5, ~50h of M15 and
~8 days of H1, with no resampling.

────────────────────────────────────────────────────────────────────────
THE ADAPTIVE CORE: ZONE RESPECT INDEX (ZRI)
────────────────────────────────────────────────────────────────────────
The bot measures, separately for demand and for supply, whether zones of
that kind are actually holding *right now* on this symbol: of the recently
resolved zones of that kind, what fraction produced a real reversal rather
than being closed straight through.  A kind whose ZRI falls under
`zri_min` is switched off until it earns its way back.

On 2026-08-05 supply zones were being obliterated from ~05:00 UTC onward
(05h 0% WR, 10h 4% WR, 11h 0% WR), so supply-ZRI would have collapsed and
suppressed the entire -$1,154 short book, while demand-ZRI stayed healthy
and left the profitable long book running.  Nothing here is hardcoded to
"gold went up on 2026-08-05" — it is a live read of which side of the
book the market is currently honouring, so it adapts to the next regime
on its own.

────────────────────────────────────────────────────────────────────────
V6 CHANGE: LOW-VOLATILITY STAND-DOWN
────────────────────────────────────────────────────────────────────────
Forensic pass on this bot's own trade history (677 closed trades,
`regime_volatility` as logged by the engine at entry time): avg R by
volatility bucket was low -0.212 (n=145) / normal +0.174 (n=272) / high
+0.658 (n=186) — high-volatility entries are consistently the strongest
performer, low-volatility the weakest, and that ranking held (though the
magnitude shrank) across both chronological halves of the history, unlike
a `regime_trend` (ranging/trending) cut that looked equally strong in
aggregate but inverted between the two halves and was discarded instead.

v5 already computes a volatility percentile every bar (`vol_pct`,
`vol_low_pct=0.20`, same 100-bar lookback and 20th-percentile cutoff
`engine.domain.volatility`'s own LOW bucket uses) and uses it only to
scale SL/TP size (`vol_sl_scale_low`/`vol_tp_scale_low`) — it never stood
the bot down in the low bucket the way `vol_extreme_block` already does
for the extreme bucket one bucket over. `vol_low_block` (new param, `True`
here, absent/off in v5) closes that gap: skip the setup entirely when
`vol_pct <= vol_low_pct`, mirroring the existing `vol_extreme_block` gate
exactly rather than inventing a new mechanism.

Honest caveat carried over from the analysis: in the *recent* half of the
history the low-volatility bucket was only marginally positive (+0.014),
not losing — so this is a trade-frequency/quality tradeoff (fewer, higher-
average-R trades), not a fix for an active leak. Compare v5 vs v6 on the
same backtest window before trusting this over the baseline.
...
def _confirmation(zone, opens, highs, lows, closes, atr_value, params):
    ...
    if body < min_body and wick_frac < min_wick:
        return False, (f"body {body:.2f}<{min_body:.2f} and wick {wick_frac:.2f}<{min_wick:.2f}")
...
class XauusdSndApexTrendguardM1V6:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            ...
            params={
                ...
                # V6: stand down entirely in the low-volatility bucket
                # instead of only shrinking SL/TP for it — see module
                # docstring's "V6 CHANGE" section for the forensic basis.
                "vol_low_block": True,
    ...
    def evaluate(self, ctx):
        ...
        if params["shock_guard_enabled"]:
            since = _bars_since_shock(
                entry["high"].to_numpy(),
                entry["low"].to_numpy(),
                int(params["shock_lookback_bars"]),
                float(params["shock_range_mult"]),
            )
        ...
        if params["vol_extreme_block"] and vol_pct >= float(params["vol_extreme_pct"]):
            return None
        if params["vol_low_block"] and vol_pct <= float(params["vol_low_pct"]):
            self._reject("vol_low_block")
            return None
