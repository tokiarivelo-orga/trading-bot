...
class XauusdSndQmStructureAdaptiveM1:
    def __init__(self):
        self.spec = StrategySpec(
            ...
            params={
                ...
                # Same-direction cooldown after a stop-out
                "cooldown_lookback_bars": 12,
                "cooldown_confluence_bump": 2,
                "cooldown_momentum_lookback": 3,
                "cooldown_rsi_neutral": 50.0,
                # Spread quality
    ...
    def _resolve_signal_history(self, entry_t_ns, entry_highs, entry_lows):
        last_ns = int(entry_t_ns[-1])
        last_high = float(entry_highs[-1])
        last_low = float(entry_lows[-1])
        for event in self._signal_history:
            if event["stop_ns"] is not None or last_ns <= event["entry_ns"]:
                continue
            if (
                event["direction"] == 1
                and last_low <= event["sl_price"]
                or event["direction"] == -1
                and last_high >= event["sl_price"]
            ):
                event["stop_ns"] = last_ns

    def _record_signal_event(self, now_ns, trade_dir, entry_price, sl_points):
        sl_price = entry_price - trade_dir * sl_points
        self._signal_history.append(
            {"entry_ns": now_ns, "direction": trade_dir, "sl_price": sl_price, "stop_ns": None}
        )
        if len(self._signal_history) > 50:
            self._signal_history = self._signal_history[-50:]

    def _direction_stopped_recently(self, trade_dir, now_ns, params):
        lookback_bars = int(params.get("cooldown_lookback_bars", 12))
        entry_minutes = int(params.get("entry_tf_minutes", 5))
        window_ns = lookback_bars * entry_minutes * _NS_PER_MINUTE
        for event in reversed(self._signal_history):
            if event["direction"] != trade_dir or event["stop_ns"] is None:
                continue
            if now_ns - event["stop_ns"] <= window_ns:
                return True
        return False

    def _momentum_turned_back(self, trade_dir, rsi_values, entry_closes, params):
        neutral = float(params.get("cooldown_rsi_neutral", 50.0))
        rsi_ok = False
        if len(rsi_values) and np.isfinite(rsi_values[-1]):
            rsi_ok = rsi_values[-1] < neutral if trade_dir == -1 else rsi_values[-1] > neutral
        lookback = int(params.get("cooldown_momentum_lookback", 3))
        structure_ok = False
        if len(entry_closes) > lookback:
            recent_ref = entry_closes[-(lookback + 1) : -1]
            if trade_dir == -1:
                structure_ok = entry_closes[-1] <= recent_ref.max()
            else:
                structure_ok = entry_closes[-1] >= recent_ref.min()
        return rsi_ok or structure_ok
    ...
    def evaluate(self, ctx: MarketContext):
        ...
        self._resolve_signal_history(entry_t_ns, entry_highs, entry_lows)
    ...
    def _decide(
        ...
        *,
        ...
        for zone, life in candidates:
            ...
            cooldown_note = ""
            if self._direction_stopped_recently(trade_dir, now_ns, params):
                if self._momentum_turned_back(trade_dir, rsi_values, entry_closes, params):
                    cooldown_note = " cd=momentum_flip"
                else:
                    bump = int(params.get("cooldown_confluence_bump", 2))
                    if confluence_score < min_confluence_score + bump:
                        continue
                    cooldown_note = f" cd=high_confluence({confluence_score})"
            ...
            scored.append((zone, life, plan, verdict, features, confluence_score, cooldown_note))

        if not scored:
            return None

        scored.sort(key=lambda item: _rank_key(item[3], item[1], item[5]), reverse=True)
        zone, life, plan, verdict, _features, confluence_score, cooldown_note = scored[0]

        trade_dir = 1 if zone["kind"] == ZoneKind.DEMAND else -1
        self._record_signal_event(now_ns, trade_dir, close, plan["sl_points"])
