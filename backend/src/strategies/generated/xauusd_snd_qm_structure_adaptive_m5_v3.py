...
class XauusdSndQmStructureAdaptiveM1:
    def __init__(self):
        self.spec = StrategySpec(
            ...
            params={
                ...
                # Re-entry cooldown: block a new signal from the same
                # direction whose zone band overlaps (within ATR tolerance)
                # a signal fired within the last N bars, whether that prior
                # position is still open or already stopped out.
                "reentry_cooldown_bars": 24,
                "reentry_cooldown_atr_mult": 0.5,
                # Spread quality
    ...
    def _recently_traded(self, zone, trade_dir, atr_val, now_ns, params):
        cooldown_ns = (
            int(params.get("reentry_cooldown_bars", 24))
            * int(params["entry_tf_minutes"])
            * _NS_PER_MINUTE
        )
        if cooldown_ns <= 0:
            return False
        tol = atr_val * float(params.get("reentry_cooldown_atr_mult", 0.5))
        zone_low = zone["price_low"] - tol
        zone_high = zone["price_high"] + tol
        for entry_ns, direction, price_low, price_high in self._recent_entries:
            if direction != trade_dir or now_ns - entry_ns > cooldown_ns:
                continue
            if zone_low <= price_high and zone_high >= price_low:
                return True
        return False

    def _register_entry(self, zone, now_ns, params):
        cooldown_ns = (
            int(params.get("reentry_cooldown_bars", 24))
            * int(params["entry_tf_minutes"])
            * _NS_PER_MINUTE
        ...
        self._recent_entries = [
            e for e in self._recent_entries if now_ns - e[0] <= cooldown_ns
        ]
        self._recent_entries.append((now_ns, trade_dir, zone["price_low"], zone["price_high"]))
    ...
    def _decide(
        ...
        *,
        ...
        for zone, life in candidates:
            ...
            if self._recently_traded(zone, trade_dir, atr_val, now_ns, params):
                continue
        ...
        self._register_entry(zone, now_ns, params)
